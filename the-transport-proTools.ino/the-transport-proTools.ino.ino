...
const int NPots = 0; //*
int potPin[NPots] = {}; //* Pin where the potentiometer is
...
byte notes[NButtons] = {70, 69, 71};
byte cc = 1; //* Lowest MIDI CC to be used

//char letter[NButtons] = {82, 84, 85}; // key codes
//char letter[NButtons] = {48, 50, 51}; // ascii codes
int letter[NButtons] = {234, 226, 227}; // key codes
    ...
    if ((millis() - lastDebounceTime[i]) > debounceDelay) {
      ...
      if (buttonPState[i] != buttonCState[i]) {
        ...
        if (buttonCState[i] == LOW) {
          ...
          //          noteOn(midiCh, notes[i], 127);  // channel, note, velocity
          //          MidiUSB.flush();
          ...
          // use if using with ATmega32U4 (micro, pro micro, leonardo...)
          //          noteOn(midiCh, notes[i], 0);  // channel, note, velocity
          //          MidiUSB.flush();
